...
describe( 'matrix cdf', function tests() {
	...
	it( 'should throw an error if provided unequal length matrices', function test() {
		...
		function badValues() {
			cdf( matrix( [10,10] ), mat, k );
		}
	});

	it( 'should evaluate the Chi-squared cdf for each matrix element', function test() {
		var actual, i;

		actual = matrix( [5,5], 'float64' );
		actual = cdf( actual, mat, k );
	...
	it( 'should return an empty matrix if provided an empty matrix', function test() {
		...
		mat = matrix( [0,10] );
		assert.deepEqual( cdf( out, mat, k ).data, expected );
