...
function cdf( x, options ) {
	...
	if ( isTypedArrayLike( x ) ) {
		...
		return cdf6( out, x, opts.k );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return cdf4( x, opts.k, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return cdf3( out, x, opts.k, opts.accessor );
		}
		return cdf2( out, x, opts.k );
	}
	return NaN;
} // end FUNCTION cdf()
